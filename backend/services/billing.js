...
export async function getRepairBillingDetails(pool, repairId) {
  ...
  try {
    ...
    const [workLogRows] = await conn.query(
      `SELECT w.label, w.time_on_repair, w.billable, w.start_work, t.hourly_rate, t.name AS technician_name
       FROM work_log w
    ...
    const [partsRows] = await conn.query(
      `SELECT pu.label, pu.quantity_used, pu.customer_cost, p.part_name
       FROM parts_used pu
